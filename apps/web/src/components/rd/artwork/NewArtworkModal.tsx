...
export interface ArtworkStepProps {
  form: ArtworkFormData
  setForm: React.Dispatch<React.SetStateAction<ArtworkFormData>>
}

// ─── Validation (canGoNext equivalent) ───────────────────────
export function canGoNext(step: number, form: ArtworkFormData): boolean {
  if (step === 0)
    return !!form.artworkName && !!form.brand && !!form.productName && form.channels.length > 0
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-subtle)]">
          <div>
            ...
            <p className="text-sm text-[var(--text-tertiary)] mt-0.5">
              Step {step + 1} of {ARTWORK_STEPS.length} &mdash; {ARTWORK_STEPS[step].label}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[var(--border-subtle)] bg-[var(--bg-base)]">
          ...
          {step < ARTWORK_STEPS.length - 1 ? (
            ...
            <button
              ...
              disabled={isSubmitting || !canGoNext(step, form)}
...
// ─────────────────────────────────────────────────────────────
// Field helpers shared across steps
// ─────────────────────────────────────────────────────────────
function makeSet(setForm: ArtworkStepProps['setForm']) {
  return <K extends keyof ArtworkFormData>(key: K, value: ArtworkFormData[K]) =>
    setForm((prev) => ({ ...prev, [key]: value }))
}

function makeToggleArrayItem(setForm: ArtworkStepProps['setForm']) {
  return (key: keyof ArtworkFormData, item: string) =>
    setForm((prev) => {
...
// ─────────────────────────────────────────────────────────────
export function ArtworkStep1({ form, setForm }: ArtworkStepProps) {
  const set = makeSet(setForm)
  const toggleArrayItem = makeToggleArrayItem(setForm)
...
// ─────────────────────────────────────────────────────────────
export function ArtworkStep2({ form, setForm }: ArtworkStepProps) {
  const set = makeSet(setForm)
  const toggleArrayItem = makeToggleArrayItem(setForm)
...
// ─────────────────────────────────────────────────────────────
export function ArtworkStep3({ form, setForm }: ArtworkStepProps) {
  const set = makeSet(setForm)
  const toggleArrayItem = makeToggleArrayItem(setForm)
...
// ─────────────────────────────────────────────────────────────
export function ArtworkStep4({ form, setForm }: ArtworkStepProps) {
  const set = makeSet(setForm)
